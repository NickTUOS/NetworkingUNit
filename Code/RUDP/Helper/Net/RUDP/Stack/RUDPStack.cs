...
namespace Helper.Net.RUDP
{
	...
	static public class RUDPStack
	{
		...
		internal const int RUDPHeaderLength = 63; // 1 + 1 + 16 + 1 + 4 + 4 + 4 + 4 * 8
												...
												int payloadLength)
		{
			...
			//---- Connection Id
			if (rudp == null)
				Buffer.BlockCopy(new byte[16], 0, packetPayload, headerOffset, 16);
			else
				Buffer.BlockCopy(rudp._connectionId, 0, packetPayload, headerOffset, 16);
			headerOffset += 16;
		...
		private static void OnEndReceive(IAsyncResult result)
		{
			...
			//-- Decode payload
			DecodePayload(physical, physical._receiveBuffer, size, sender as IPEndPoint,
							out connectionId,
							out channel,
							out packetId,
			...
			HandlePacket(physical, sender as IPEndPoint, connectionId, channel, packetId, advertisedWindowSize, slot1, slot2, slot3, slot4, payload);
		}

		#endregion

		#region DecodePayload

		/// <summary>
		/// Handle the bytes received from a socket.
		/// </summary>
		private static void DecodePayload(PhysicalSocket physical, byte[] payload, int length, IPEndPoint sender,
				out byte[] connectionId,
				...
				out byte[] packetPayload)
		{
			...
			Buffer.BlockCopy(payload, offset, connectionId, 0, 16);
			offset += 16;
										...
										byte[] payload)
		{
			...
			//---- Connection Id /  NAT Handling management
			
			RUDPSocket rudp2 = null;
			physical._connectedIdsLock.EnterReadLock();
			physical._connectedIds.TryGetValue(connectionId, out rudp2);
			physical._connectedIdsLock.ExitReadLock();
			
			if (rudp2 == null)
			{
				// New connection (Should be ping channel)
				if (rudp != null)
				{
					physical._connectedIdsLock.EnterWriteLock();
					physical._connectedIds.Add(connectionId, rudp);
					physical._connectedIdsLock.ExitWriteLock();
				}
				else
					return; // Cannot handle this case !!!!
			}
			else
			{
				// Check if the IP has not changed
				if (rudp == null && rudp2 != null)
					if (!rudp2._remoteEndPoint.Equals(sender))
						{
							// Add to history
							rudp._NATEndPointsHistory.Add(new RemoteMappingChangePoint(DateTime.Now, rudp));

							// Update the socket
							physical._connectedRDUPsLock.EnterWriteLock();
							physical._connectedRDUPs.Remove(rudp._remoteEndPoint);
							physical._connectedRDUPs.Add(sender, rudp);
							rudp._remoteEndPoint = sender;
			...
			{
				int greaterPacketId = rudp._incomingPackets.BiggestPacketId;
				...
				for (int index = 0; index < rudp._outgoingPackets.Count; index++)
				{
					...
					if (slot4 != null)
						if (packet.PacketId >= slot4.StartPacketId && packet.PacketId <= slot4.EndPacketId)
						{
							...
							{
								lastPacket = packet;
								currentRTT = Math.Min(currentRTT, now - lastPacket.TSFirstSend);
							}
		...
		private static void ControlTimerProcessing(ControlThreadInformation controlInformation)
		{
			while (Thread.CurrentThread.IsAlive && _isStackRunning)
			{
				...
				int rudpCount = controlInformation._rudpSockets.Count - 1;

				if (rudpCount < 0)
					Thread.Sleep(1);
				else
				{
					for (int index = rudpCount; index > -1; index--)
					{
