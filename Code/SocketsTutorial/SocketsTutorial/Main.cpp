...
int main()
{
	...
	while (true) // forever
	{
		...
		if ((clientSocket = accept(serverSocket, (SOCKADDR*)&clientAddr, &clientAddrSize)) != INVALID_SOCKET) //if a new connection exists, accept it and create a new socket																										//we bind server soceck and client addr to client socket as this combonation makes the socket
		{
